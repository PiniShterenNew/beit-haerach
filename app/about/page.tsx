    ...
    <>
      <Section className="pt-12 md:pt-16">
        ...
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          <div className="flex flex-col gap-5 text-base leading-relaxed text-(--color-text-secondary)">
            <p>
      ...
      <Section>
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          <TrustBadge icon="home" title="כבוד אנושי" description="כל פנייה נענית מתוך כבוד, ולא מתוך רחמים או תיוג." />
          <TrustBadge icon="calendar" title="המשכיות" description="פעילות קבועה, לא נקודתית — כל השנה, כולל שבתות וחגים." />
          <TrustBadge icon="community" title="קהילה" description="מענה שמחבר בין תחומים — מזון, בריאות, תורה וחינוך." />
          <TrustBadge icon="transparency" title="שקיפות" description="דיווח פתוח על הפעילות ועל האופן שבו נעשה שימוש בתרומות." />
        </div>
      </Section>
      ...
      <Section tone="muted">
        ...
        <ul className="grid gap-6 sm:grid-cols-2">
          {PROGRAMS.map((p) => (
            <li key={p.id} className="flex items-start justify-between gap-4 border-b border-(--color-border-subtle) pb-6">
              <div>
